  ...
  return (
    <>
      ...
      <section style={{ paddingTop: '1.5rem', paddingBottom: '0.5rem' }}>
        <p className="landing-intro-line">{introPriceLine}</p>
      </section>

      {/* Hero */}
      <section style={{ paddingTop: '0.5rem', paddingBottom: '2rem', textAlign: 'center' }}>
        <h1 style={{ fontSize: 'clamp(1.75rem, 4vw, 2.5rem)', fontWeight: 600, marginBottom: '0.5rem' }}>
          {hero.title}
        </h1>
        <p style={{ fontSize: 'clamp(1.05rem, 2vw, 1.2rem)', color: 'var(--text)', maxWidth: 520, margin: '0 auto 0.5rem' }}>
          {hero.headline}
        </p>
        <p style={{ color: 'var(--text-secondary)', fontSize: '1rem', maxWidth: 480, margin: '0 auto 2rem' }}>
          {hero.subtitle}
        </p>
      ...
      <section style={{ borderTop: '1px solid var(--divider)' }}>
        <h2 style={{ fontSize: '1.5rem', fontWeight: 600, marginBottom: '0.35rem', textAlign: 'center' }}>
          {whyChoose.title}
        </h2>
        ...
        >
          {testimonials.items.map((t, i) => (
            <div key={i} className="landing-testimonial-card">
              ...
              <p style={{ fontSize: '0.95rem', lineHeight: 1.5 }}>&ldquo;{t.quote}&rdquo;</p>
      ...
      <section style={{ borderTop: '1px solid var(--divider)' }}>
        <div className="landing-pricing-box">
          <h2 style={{ fontSize: '1.35rem', fontWeight: 600, marginBottom: '0.5rem' }}>{pricingCta.title}</h2>
          ...
          {pricing.price && (
            <p style={{ marginBottom: '0.5rem' }}>
              ...
              {pricing.wasPrice ? <span className="landing-price-was">{pricing.wasPrice}</span> : null}
              {pricing.savePercent > 0 ? (
                <span className="landing-price-save">Save {pricing.savePercent}%</span>
              ) : null}
            </p>
          )}
